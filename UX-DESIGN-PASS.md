...
- [~] Phase 3 — **axis workbench 3a** ✅ (selectable axes: measures OR a **theme on each axis for a 2×2 quadrant** with quadrant labels/counts + bridge emphasis; jittered, filter-aware) · **tag-overlap affinity** ✅ (clusters cards sharing ≥2 themes via connected components, multi-theme cluster signatures) · [ ] 3b manual axis placement (needs a per-card axis-value column — schema step)
